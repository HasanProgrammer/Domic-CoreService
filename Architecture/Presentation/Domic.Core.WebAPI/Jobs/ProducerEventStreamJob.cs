...
public class ProducerEventStreamJob : IHostedService, IDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        var currentServiceJob = $"{ _externalEventStreamBroker.NameOfService }-{ nameof(ProducerEventStreamJob) }";
        ...
        _timer =
            new Timer(state => {
                ...
                _externalEventStreamBroker.PublishAsync(cancellationToken);

                var nowDateTime = DateTime.UtcNow;

                _hubContext.Clients.Groups("JobsSignalGroup").SendAsync(
                    new JobsSignalDto {
                        Service = _externalEventStreamBroker.NameOfService,
                        Title = "ProducerEventStreamJob",
